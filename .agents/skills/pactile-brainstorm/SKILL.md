...
**Artifact language:** Read `.pactile/spec/guides/artifact-locale-guide.md`. Write planning artifacts in the resolved `artifact_locale` (workspace default or task override from `task.py artifact-locale get`).
...
4. **Platform** — project platform hooks, agents, and skills (see `.pactile/spec/guides/cursor-subagent-policy.md` when using Cursor); shared `.agents/skills/` when behavior spans platforms.
5. **Parent/Child** — if multiple independent deliverables, note child split early in `prd.md`.

Use retrieval per `.pactile/spec/guides/retrieval-daily-guide.md` (rg for literals, codegraph for structure, fast-context for semantic sweep).
...
|   | Link: `.pactile/spec/guides/durable-learning-decision-guide.md` § Valid outcomes |
