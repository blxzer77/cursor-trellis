...
- select a task with `python ./.pactile/scripts/task.py select <task>`
- create a task
- inspect details
...
When a task is selected, `get_context.py` shows the selected task's `status` field. Route by `status` + artifact presence. This command replaces the user needing to remember the Pactile flow; it does not itself approve implementation.

- `status=planning` + no `prd.md` → **1.1** (load `pactile-brainstorm`)
...
Phase rules (full detail in `.pactile/workflow.md`):
...
```bash
python ./.pactile/scripts/get_context.py --mode phase --step <X.X> --platform cursor
```
...
Full workflow and detailed phase steps live in `.pactile/workflow.md`. This command is only an entry point — the canonical guidance is there.
