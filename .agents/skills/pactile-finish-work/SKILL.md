...
Before archive, confirm `verify.md` includes Phase 3.3 **Learning decision** (`update-spec` | `no-update` | `unsure`) and gate-compatible evidence per `.pactile/spec/guides/durable-learning-decision-guide.md`. Run `task.py archive <task> --check` when unsure.
...
```bash
python ./.pactile/scripts/get_context.py --mode record
```
...
- **Any remaining path looks like selected-task work** — bail out with:
  > "Working tree has uncommitted code changes from this task: `<list>`. Return to workflow Phase 3.4 to commit them before running `pactile-finish-work`."
...
```bash
python ./.pactile/scripts/task.py archive <task-name>
```
...
```bash
python ./.pactile/scripts/add_session.py \
  --title "Session Title" \
  --commit "hash1,hash2" \
