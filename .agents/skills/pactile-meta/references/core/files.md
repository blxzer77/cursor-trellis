# Pactile File Reference

Complete reference of all files in the `.pactile/` directory.
...
├── .version                # Installed Pactile version
...
```json
{
  "current_task": ".pactile/tasks/01-31-add-login-taosu",
  "current_run": null,
  "platform": "cursor",
...
**Purpose**: Legacy ignored pointer from older Pactile versions.

**Active-task behavior**: Not read or written as a fallback. Current Pactile
...
**Created by**: Legacy Ralph Loop tooling (not shipped in pactile; file may remain gitignored if present from older upgrades)

**Format**: JSON

```json
{
  "task": ".pactile/tasks/01-31-add-login",
  "iteration": 2,
...
**Purpose**: Track template file versions for `pactile update`.

**Created by**: `pactile init` or `pactile update`

**Format**: JSON object mapping file paths to SHA-256 hashes.

```json
{
  ".pactile/workflow.md": "028891d1fe839a266...",
  ".cursor/hooks/session-start.py": "0a9899e80f6bfe15...",
  ".cursor/commands/pactile-start.md": "d1276dcbff880299..."
}
```

**Used by**:
- `pactile update` - Detect which files have been modified
...
**Purpose**: Track installed Pactile CLI version.
...
**Used by**:
- `pactile update` - Determine if update is needed
...
**Location**: `.pactile/workspace/{developer}/.agents/`
...
**Created by**: Not used by pactile Cursor hooks.
...
These files are managed by `pactile update`:

| File | Purpose |
|------|---------|
| `.pactile/workflow.md` | Workflow documentation |
| `.pactile/worktree.yaml` | Multi-session config |
| `.pactile/.gitignore` | Git ignore rules |
...
.pactile/tasks/{task}/ → .pactile/tasks/archive/YYYY-MM/
