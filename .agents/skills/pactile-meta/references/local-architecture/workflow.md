...
`.pactile/workflow.md` is the Pactile workflow source of truth inside the user project. An AI does not need Pactile source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.pactile/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch `pactile-implement` by default for implementation and `pactile-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `pactile-before-dev`, then executes directly.
...
After editing, make the AI reread `.pactile/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `pactile-brainstorm` and `pactile-check`.
