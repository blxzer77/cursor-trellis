...
Common Pactile customization scenarios and what files need to be modified.
...
|------|-----------------|----------|
| [Add slash command](#add-slash-command) | commands/, pactile-local | All |
| [Add agent](#add-agent) | agents/, hook, jsonl, pactile-local | CC |
| [Modify hook](#modify-hook) | hooks/, settings.json, pactile-local | CC |
| [Add spec category](#add-spec-category) | spec/, jsonl, pactile-local | All |
| [Change verify commands](#change-verify-commands) | worktree.yaml | CC |
| [Add workflow phase](#add-workflow-phase) | task.json, dispatch, pactile-local | CC |
| [Add post_create step](#add-post_create-step) | worktree.yaml | CC |
| [Modify session start](#modify-session-start) | session-start.py, pactile-local | CC |
| [Add core script](#add-core-script) | scripts/, pactile-local | All |
...
**Scenario**: Add a new `/pactile:my-command` command.

**Files to modify**:

```
.claude/commands/pactile/my-command.md    # Create: Command prompt
.cursor/commands/my-command.md            # Create: Mirror for Cursor (optional)
.pactile-local/SKILL.md                   # Update: Document the change
```

**Steps**:
1. Create command file with YAML frontmatter
2. Mirror to Cursor if needed
3. Document in pactile-local
...
.pactile/tasks/{template}/my-agent.jsonl            # Create: Context template
.pactile-local/SKILL.md                             # Update: Document the change
```
...
```
.pactile/spec/mobile/index.md             # Create: Category index
.pactile/spec/mobile/*.md                 # Create: Spec files
.pactile/tasks/{template}/*.jsonl         # Update: Reference new specs
.pactile-local/SKILL.md                   # Update: Document the change
```
...
```
.pactile/worktree.yaml                    # Modify: verify section
```
...
```
.pactile/worktree.yaml                    # Modify: post_create section
```
...
```
.pactile/scripts/my-script.py             # Create: Script
.pactile/scripts/common/*.py              # Create/Modify: If shared utilities
.pactile-local/SKILL.md                   # Update: Document the change
```
...
```
.pactile/scripts/task.py                  # Modify: init-context logic
.pactile/tasks/{template}/*.jsonl         # Create: New JSONL templates
.pactile-local/SKILL.md                   # Update: Document the change
```
