...
This page lists Pactile file locations in a user project on **Cursor**.

Pactile originally supported multiple AI platforms (Claude Code, Codex, OpenCode, Kilo, Kiro, Gemini, Antigravity, Windsurf, Qoder, CodeBuddy, GitHub Copilot, Factory Droid, Pi Agent). The project has since converged on **Cursor-only**. Legacy adapter directories may still exist in upgraded projects (`pactile update` preserves user files), but new framework/runtime behavior targets Cursor exclusively.
...
- **Agents** (`pactile-research`, `pactile-implement`, `pactile-check`) live in `.cursor/agents/` and are dispatched via the Task tool or opened as Agent sessions / Skill forms depending on the entry point (see `cursor-subagent-policy.md.txt`).
- **Commands** (e.g. `record-session`, `pactile-continue`) live in `.cursor/commands/` and are user-invocable via `/slash`.
...
3. User only says "my AI": inspect which directories actually exist in the project; on a fresh `pactile init` only `.cursor/` and `.pactile/` are created.
4. User wants project rules: prefer `.pactile/spec/` or a project-local skill under `.cursor/skills/`.
5. User wants Pactile behavior: edit `.pactile/workflow.md` plus `.cursor/hooks.json` / `.cursor/agents/` / `.cursor/skills/` / `.cursor/commands/`.
...
- `pactile update` preserves them; do not delete on sight.
...
- Do not register new hooks/agents/skills/commands under legacy platform directories. New Pactile features ship to `.cursor/` only.
