...
Update to Trellis itself → Update cstl-meta (meta skill)
```
...
# Copy template from cstl-meta/references/trellis-local-template.md
```
...
~/.claude/skills/
└── cstl-meta/                        # Shared meta-skill (vanilla Trellis)
```
...
diff -r ~/.claude/skills/cstl-meta/references/ \
        ./new-cstl-meta/references/
```
...
cp -r ~/.claude/skills/cstl-meta ~/.claude/skills/cstl-meta.backup

# Update meta-skill
cp -r ./new-cstl-meta/* ~/.claude/skills/cstl-meta/
```
...
**Never** modify `cstl-meta` for project-specific changes.
