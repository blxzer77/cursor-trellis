...
| --- | --- |
| Implement agent must follow extra restrictions | The platform's `cstl-implement` agent file. |
| Check agent must run project-specific commands | `cstl-check` agent file, and `.trellis/spec/` if needed. |
| Research agent must output a fixed format | `cstl-research` agent file. |
