...
const RETRIEVAL_SOURCE_EXTENSIONS = new Set([".ts", ".py", ".md", ".json"]);

function walk(root: string): string[] {
  return readdirSync(root, { withFileTypes: true }).flatMap((entry) => {
    const target = path.join(root, entry.name);
    if (entry.isDirectory()) {
      return entry.name.toLowerCase() === "__pycache__" ? [] : walk(target);
    }
    return /\.py[co]$/iu.test(entry.name) ? [] : [target];
  });
}
...
describe("Pactile retrieval V3 live-symbol guard", () => {
  ...
  it("ignores generated bytecode while scanning every retrieval source extension", () => {
  ...
  it("keeps the Python and TypeScript ABI version fixed at V3", () => {
    ...
    expect(python).toMatch(/ROUTER_VERSION\s*=\s*3/);
