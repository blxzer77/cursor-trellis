...
import { reportInitReadiness } from "../../src/utils/readiness.js";

describe("reportInitReadiness", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("warns unverified and does not throw when smoke failed", () => {
    ...
    expect(text).toMatch(/official files can still be written/);
    expect(text).toMatch(/codebase-retrieval capability unverified/);
    expect(text).not.toMatch(/can still update after confirmation/);
  });
  ...
  it("keeps --skip-readiness as an unverified maintainer hatch", () => {
    ...
    expect(text).not.toMatch(/Selected project capability readiness skipped/);
  });
});
