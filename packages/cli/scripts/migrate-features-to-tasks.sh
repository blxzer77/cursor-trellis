...
# - .pactile/workspace/{developer}/features/{name}/ -> .pactile/tasks/{MM}-{DD}-{name}-{developer}/
...
PACTILE_DIR="$PROJECT_DIR/.pactile"

# Check if .pactile exists
if [[ ! -d "$PACTILE_DIR" ]]; then
    echo "Error: .pactile directory not found in $PROJECT_DIR"
    exit 1
fi

# Determine the workspace directory (could be workspace or agent-traces if not yet updated)
if [[ -d "$PACTILE_DIR/workspace" ]]; then
    WORKSPACE_DIR="$PACTILE_DIR/workspace"
elif [[ -d "$PACTILE_DIR/agent-traces" ]]; then
    WORKSPACE_DIR="$PACTILE_DIR/agent-traces"
    echo "Warning: Found agent-traces instead of workspace. Run 'pactile update' first."
