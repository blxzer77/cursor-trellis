...
function binaryCheckCommand(
  ...
  return platform === "win32" ? "where pactile" : "which pactile";
}
...
function troubleshooting(plan: UpgradeCommandPlan): string {
  return [
    ...
    `- Check which Pactile binary your shell resolves: ${plan.binaryCheckCommand}`,
    "- If this is a permissions error, fix your Node/npm install or npm prefix; Pactile does not run sudo.",
    "- If npm reports an existing binary or locked file, resolve that npm error manually; Pactile does not run --force.",
  ].join("\n");
}
...
export async function upgrade(
  ...
  console.log(chalk.cyan(`Upgrading Pactile CLI to ${plan.target}`));
  ...
  console.log(chalk.green("\n✓ Pactile CLI upgrade completed"));
  console.log(chalk.gray("Run: pactile --version"));
