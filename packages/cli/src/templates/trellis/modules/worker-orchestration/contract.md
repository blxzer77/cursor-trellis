...
执行要求的**派工绑定**，不是生命周期槽，也不是调度器。当合同要求 worker / 隔离 / 并行时，把「谁去做」绑到 Adapter 能提供的类型化工人（implement / check / research）。Prefer 宿主原生并行（可见并行面 / 类型化工人 / 原生隔离）；CSTL 只写契约与派工包，不造 Trellis 调度器、不跨平台把别的 IDE 当工人。主会话是唯一 dispatcher；工人不是 Kernel Phase。
