---
name: pactile-micro-grill
description: "Clarifies small underspecified Pactile requests with one high-value question at a time before direct execution. Use when a request is likely small, missing details would materially change the result, and creating a Pactile task would be premature."
---
...
Clarify a small request just enough to execute it directly without creating Pactile task artifacts by default.
...
- Never create Pactile task artifacts by default.
...
1. Confirm internally that the request is small, underspecified, and not already covered by a selected Pactile task.
...
When escalating, hand the clarified answers into `pactile-brainstorm` and update Pactile artifacts there.
...
- Do not use when there is already a selected Pactile task for the work.
