...
4. Whether project rules already exist in `.pactile/spec/`
...
| Team project conventions | Prefer `.pactile/spec/` or a project-local skill. |
| Change Pactile flow semantics | Synchronize `.pactile/workflow.md`. |
...
For detailed skill authoring and review rules, use `pactile-skill-creator`. This `pactile-meta` page only identifies where local Pactile skill, command, prompt, and workflow changes belong.
...
- Which `.pactile/` files to read.
- Which scripts to run.
- How to report after completion.

If a command only repeats workflow rules, prefer making it reference/read `.pactile/workflow.md` instead of maintaining a second copy of the flow.
...
```text
.cursor/skills/project-pactile-local/
└── SKILL.md
```

For shared skill layers across tools, consider `.agents/skills/` on platforms that read that path — but do not extend new Pactile behavior there; new Pactile behavior ships to `.cursor/skills/`.
...
- Do not hide long-term engineering conventions inside a command; write them to `.pactile/spec/`.
