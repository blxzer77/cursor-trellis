...
Use a single agent as the default execution model. The agent may create Pactile tasks for traceability, but the skill should not require a specific platform, CLI, or parallel worker model.
...
When a Pactile task is useful, write a concise PRD with these sections:

```markdown
# Fill <package-or-layer> Pactile Specs

## Goal
Write project-specific `.pactile/spec/` guidance for <scope>.
...
- `.pactile/spec/.../index.md`
- `.pactile/spec/.../<topic>.md`
