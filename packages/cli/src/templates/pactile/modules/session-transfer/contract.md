...
把**还在飞的工作**搬到另一个会话 / 人 / 宿主 / 目录。产出一份一次性过境文档（默认写 OS temp，不进仓库、不当任务证据）。`/pactile-handoff` 是 Adapter 的平台入口；本块拥有正文契约（何时搬、写什么、不写什么）。与 Child 的 `integration-handoff`（任务目录里的集成证据）分 owner。不是 Continue，不是 Compact，不是派工包。
...
4. 不得写入 `.pactile/tasks/<dir>/handoff.md` 冒充集成证据；不得把过境文档当长期 spec。
