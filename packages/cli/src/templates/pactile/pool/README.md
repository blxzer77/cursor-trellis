...
- `depends_on: ["pool:Pxx"]` uses `delivery`, not linked-task Close: standing / landed / deferred → SATISFIED; open / in-slice → NOT_SATISFIED; missing `.pactile/pool/` (no Capture) → ignored (does not block Close); missing item → UNRESOLVED.
...
| Command | Role | Exit |
| --- | --- | --- |
| `python3 ./.pactile/scripts/pool.py validate` | Four-pack / required keys / status / delivery / unique id / dangling links / bidirectional consistency | error-level → 1 |
| `python3 ./.pactile/scripts/pool.py plan-check` | plan.md referenced ids exist (ghost id → error) | same |
| `python3 ./.pactile/scripts/pool.py link <item-id> <task-ref>` | Bidirectional link (idempotent) | missing → 1 |
| `python3 ./.pactile/scripts/pool.py unlink <item-id> <task-ref>` | Bidirectional unlink | item missing → 1 |
| `python3 ./.pactile/scripts/pool.py show <item-id>` | Print item + delivery + linked task status summary | missing → 1 |
| `python3 ./.pactile/scripts/pool.py status` | Print delivery groups (queue / standing / closed) | 0 |
...
All commands accept `--root <path>` when the Pactile root is not the current working directory.
...
Priority is **attention + start preference**, not a queue lock. Isolatable write-sets still run in parallel (see `.pactile/workflow.md` Parallel-first). Agent proposes; user chooses. Agent must not auto-start, and must not refuse an independent item because a higher band is unfinished.

1. Run `python3 ./.pactile/scripts/pool.py status`. Rank **only** `open` and `in-slice`.
...
When this repo is PACTILE itself: the workflow package and optional BYOK are **independent ships**. See `.pactile/framework/release-boundary.md`. BYOK readiness is not a workflow-package gate.
