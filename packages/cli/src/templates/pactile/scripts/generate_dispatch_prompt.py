#!/usr/bin/env python3
"""
CLI Layer 2 — PRIMARY (guaranteed) context path for pactile-* Task dispatch.

Usage:
    python ./.pactile/scripts/generate_dispatch_prompt.py --agent research
    python ./.pactile/scripts/generate_dispatch_prompt.py --agent implement --task .pactile/tasks/06-23-subagent-smoke
    python ./.pactile/scripts/generate_dispatch_prompt.py --agent research --repo-root d:/MyHarness
...
`Task(subagent_type=pactile-*, prompt=<generated>)`. This is the ONLY
...
The generated prompt carries:
  - Role identity ("You are the Pactile Research/Implement/Check Agent")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a complete Pactile subagent dispatch prompt."
    )
    parser.add_argument(
        "--agent",
        required=True,
    ...
    parser.add_argument(
        "--repo-root",
        default=None,
        help="Repository root path. If omitted, auto-detect via .pactile or .git.",
    )
    parser.add_argument(
        "--finish",
        action="store_true",
    ...
    if args.repo_root:
        ...
        if not repo_root:
            print("ERROR: could not find repo root (no .pactile or .git upward)", file=sys.stderr)
            return 1
    ...
    if args.task:
        ...
        if not task_dir:
            print(
                ...
                "Run `python ./.pactile/scripts/task.py select <task>` first.",
                file=sys.stderr,
            )
            return 1
...
def _find_repo_root(start: Path) -> Path | None:
    ...
    while current != current.parent:
        if (current / ".pactile").is_dir() or (current / ".git").exists():
            return current
