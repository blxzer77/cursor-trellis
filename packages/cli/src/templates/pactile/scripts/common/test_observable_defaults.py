...
def test_write_seed_jsonl_uses_real_spec_paths(tmp_path: Path) -> None:
    ...
    spec_root = repo_root / ".pactile" / "spec" / "guides"
    spec_root.mkdir(parents=True)
    (spec_root / "index.md").write_text("# index.md\n", encoding="utf-8")
    framework_root = repo_root / ".pactile" / "framework"
    ...
    task_dir = repo_root / ".pactile" / "tasks" / "08-07-test-seed"
    ...
    assert all(row["file"].startswith(".pactile/spec/") or row["file"].startswith(".pactile/framework/") for row in rows)
...
def test_resolve_seed_spec_paths_prefers_existing_guides(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    guides = repo_root / ".pactile" / "spec" / "guides"
    ...
    task_dir = repo_root / ".pactile" / "tasks" / "08-07-test-paths"
    task_dir.mkdir(parents=True)
...
def test_format_verify_summary_ok_when_all_signals(tmp_path: Path) -> None:
    ...
    (task_dir / "verify.md").write_text(
        "\n".join(
            [
                "Validation commands: pytest -q — pass",
                "Check evidence: pactile-check — pass",
...
def test_render_task_dashboard_includes_verify_summary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    tasks_dir = repo_root / ".pactile" / "tasks"
