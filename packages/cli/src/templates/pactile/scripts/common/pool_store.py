#!/usr/bin/env python3
"""
Review-pool data access layer (`.pactile/pool/`).
...
@dataclass
class PoolItem:
    """One parsed pool entry from `.pactile/pool/items/`."""
...
def get_pool_root(repo_root: Path | None = None) -> Path:
    """Return the `.pactile/pool/` directory for the repository."""
    if repo_root is None:
        repo_root = get_repo_root()
    return repo_root / ".pactile" / "pool"


def list_items(repo_root: Path | None = None) -> list[PoolItem]:
    """Parse every entry in `.pactile/pool/items/`, sorted by id."""
    if repo_root is None:
        repo_root = get_repo_root()
