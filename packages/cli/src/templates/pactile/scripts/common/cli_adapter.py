...
Cursor-only fork: abstracts Pactile paths and command layout for Cursor IDE.
...
class CLIAdapter:
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        if len(parts) >= 2 and parts[0] == "pactile":
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"pactile-{filename}"
            )
        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_pactile_command_path(self, name: str) -> str:
        return f".cursor/commands/pactile-{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    override = __import__("os").environ.get("PACTILE_PLATFORM", "").strip().lower()
    if override == "cursor":
        return "cursor"
