#!/usr/bin/env python3
"""
Pactile configuration reader.

Reads settings from .pactile/config.yaml with sensible defaults.
"""
...
def _strip_inline_comment(value: str) -> str:
    ...
    Mirrors :func:`common.pactile_config._strip_inline_comment` so both
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Set ``session_auto_commit: false`` in ``.pactile/config.yaml`` to skip
...
def get_journal_snippet_enabled(repo_root: Path | None = None) -> bool:
    ...
    Set ``context_injection.journal_snippet: true`` in ``.pactile/config.yaml`` to
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .pactile/.
