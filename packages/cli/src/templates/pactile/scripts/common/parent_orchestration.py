    ...
    if mode == "subagent":
        ...
        lines.extend(
            [
                ...
                "- **Default:** Parent session dispatches **Task** with `subagent_type=pactile-implement` and this prompt as the task description (writable sub-agent). Native Cursor `model:` frontmatter still applies.",
                ...
                "- Parent retains `review-child` / `integrate-child`. Child must not nest further `pactile-research` / `pactile-implement` / `pactile-check` Task dispatches.",
