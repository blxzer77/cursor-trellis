...
No writes under .pactile/.runtime/. Optional per-machine override:
PACTILE_SMART_SEARCH_COMMAND or smart_search.command in config.yaml.
"""
...
def _resolve_config_or_env(repo_root: Path) -> list[str] | None:
    env_cmd = os.environ.get("PACTILE_SMART_SEARCH_COMMAND", "").strip()
    if env_cmd:
...
def _optional_repo_wrappers(repo_root: Path) -> list[list[str]]:
    ...
    candidates: list[Path] = [
        ...
        / "bin"
        / "smart-search.js",
        repo_root / "pactile" / "packages" / "cli" / "bin" / "smart-search.js",
        repo_root / "Pactile" / "packages" / "cli" / "bin" / "smart-search.js",
