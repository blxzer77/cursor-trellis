"""
Safe git-add helpers for Pactile-owned paths.

Why this module exists
----------------------
A real user incident: a project's `.gitignore` listed `.pactile/` (company-wide
...
`git add -f .pactile/` — which fan-out-included every ignored subtree
(`.pactile/.backup-*/`, `.pactile/worktrees/`, `.pactile/.template-hashes.json`,
`.pactile/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.pactile/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.pactile/` in `.gitignore` is treated as user
  intent ("keep .pactile/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .pactile/` ...``
...
# Paths under .pactile/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.pactile/` tree.
PACTILE_IGNORED_SUBPATHS = (
    ".pactile/.backup-*",
    ".pactile/worktrees/",
    ".pactile/.template-hashes.json",
    ".pactile/.runtime/",
    ".pactile/.cache/",
)


def safe_pactile_paths_to_add(repo_root: Path) -> list[str]:
    ...
    Included:
      - .pactile/workspace/<developer>/journal-*.md
      - .pactile/workspace/<developer>/index.md
      - .pactile/tasks/<task-dir>/   (every active task directory)
      - .pactile/tasks/archive/      (whole archive subtree, if present)

    Excluded (intentionally — these must not be staged):
      - .pactile/.backup-*, .pactile/worktrees/,
        .pactile/.template-hashes.json, .pactile/.runtime/, .pactile/.cache/
    """
    paths: list[str] = []
...
def safe_archive_paths_to_add(
    ...
    `.pactile/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .pactile/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .pactile/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    print(
        "[WARN] Pactile manages these specific paths and they should be tracked:",
        file=sys.stderr,
    )
    if paths:
        for p in paths:
        ...
        print(
            "[WARN]   .pactile/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .pactile/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .pactile/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.pactile/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in PACTILE_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .pactile/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .pactile/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .pactile/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
