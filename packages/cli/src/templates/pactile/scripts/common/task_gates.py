...
CHECK_EVIDENCE_RE = re.compile(
    r"(?im)^\s*(?:[-*]\s*)?(?:check\s+evidence|pactile-check(?:\s+evidence)?)"
    r"\s*:\s*(\S[^\r\n]*)$"
)
...
AUTO_PLANNING_REVIEWER = "pactile-cli"
...
def wave_c_stop_read_confirmed(start: Path | None = None) -> bool:
    """Return True after ``pactile update`` wrote the Wave C confirm flag.

    Unconfirmed projects keep leftover dual-read. Missing ``.pactile`` is
...
def suggest_spec_targets(repo_root: Path, task_dir: Path, task_data: dict) -> list[str]:
    ...
    spec_root = repo_root / ".pactile" / "spec"
    ...
    if isinstance(package, str) and package.strip():
        ...
        if pkg_index.is_file():
            suggestions.append(f".pactile/spec/{package.strip()}/index.md")
    ...
    if isinstance(scope, str) and scope.strip():
        ...
        if scope_path.is_dir():
            ...
            if index.is_file():
                suggestions.append(
                    f".pactile/spec/{scope_path.relative_to(spec_root).as_posix()}/index.md"
                )
    ...
    for rel in guide_paths:
        path = spec_root / rel
        if path.is_file():
            suggestions.append(f".pactile/spec/{rel}")
...
def build_spec_update_scaffold(
    ...
    *,
    ...
    rel_task = f".pactile/tasks/{task_dir.name}"
    lines = [
    ...
    lines.extend(
        [
            ...
            "   - Reusable insight: `Spec update evidence: .pactile/spec/<path>` after edits",
            f"   - Already documented: `Learning artifact: {rel_task}/handoff.md`",
            "",
            "2. Use `/pactile:update-spec` or `/pactile:break-loop` for depth; never auto-write specs.",
            "",
        ]
    )
    if targets:
        ...
        lines.append(
            "3. Browse `.pactile/spec/<package-or-layer>/index.md` for the right code-spec file."
        )
        lines.append("")
    lines.append(
        "4. Re-run `python ./.pactile/scripts/task.py archive <task> --check` after verify.md is final."
    )
    lines.append("")
    return "\n".join(lines)
...
def _learning_decision_draft_lines(task_dir: Path, task_data: dict) -> list[str]:
    ...
    targets = suggest_spec_targets(repo_root, task_dir, task_data)
    target_hint = targets[0] if targets else ".pactile/spec/<layer>/index.md"
    return [
        ...
        f"# Learning artifact: .pactile/tasks/{task_dir.name}/handoff.md",
        "# Spec update needed: (brief reason) — then run /pactile:update-spec and point Spec update evidence at the edited file",
        "",
    ]


ARCHIVE_EVIDENCE_DRAFT_MARKER = "<!-- pactile:archive-evidence-draft -->"
...
def archive_repair_hints(
    ...
    rel_task = f".pactile/tasks/{task_ref}"

    for error in errors:
        if error == "verify.md":
            hints.append(
                f"Create verify.md, then run: python ./.pactile/scripts/task.py "
        ...
        if error == "verify.md missing validation evidence":
            hints.append(
                ...
                f"or run: python ./.pactile/scripts/task.py prepare-archive-evidence {rel_task}"
            )
            continue
        ...
        if error == "verify.md missing durable-learning decision evidence":
            hints.append(
                ...
                "'Spec update evidence: .pactile/spec/<path>' after /pactile:update-spec; "
        ...
        if error.startswith("missing gate record:"):
            ...
            if transition == "parent-integrated":
                hints.append(
                    ...
                    f"python ./.pactile/scripts/task.py record-gate {rel_task} "
        ...
        if error == "verify.md missing check evidence":
            hints.append(
                "Add a grep-friendly line such as "
                "'Check evidence: <pactile-check summary or manual review notes>' to verify.md"
            )
            continue
        if error == (
            "verify.md or handoff.md missing reviewed change-set evidence"
    ...
    if not hints and not guard.ok:
        hints.append(
            f"Run: python ./.pactile/scripts/task.py prepare-archive-evidence {rel_task} "
    ...
    for error in errors:
        if error.startswith("missing gate record:"):
            ...
            if gate in START_EXECUTION_AUTO_GATES:
                hints.append(
                    "Complete planning artifacts so Pactile can auto-record this gate on "
        ...
        if "Acceptance Criteria" in error or "TBD" in error:
            hints.append(
                "Finish PRD Grill in pactile-brainstorm: fill Goal, Acceptance Criteria "
