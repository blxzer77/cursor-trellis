...
def _collect_package_git_info(
    ...
    When the Pactile root is not a Git repo and no configured package repos are
...
def _fetch_pactile_version_output() -> str | None:
    try:
...
def _resolve_available_update_version() -> str | None:
    output = _fetch_pactile_version_output()
    if not output:
        return None
...
def _get_update_hint(repo_root: Path) -> str | None:
    ...
    return (
        f"Pactile update available: {current_version} -> {latest_version}, "
        "run pactile upgrade"
    )
...
def _get_retrieval_guide(
    ...
    guide: dict[str, object] = {
        "artifactSearch": {
            ...
            "purpose": (
                "Find durable Pactile specs, tasks, research, verification "
    ...
    recommendations.extend(
        [
            _recommendation(
                ...
                90,
                "high",
                "Search durable Pactile specs, tasks, research, and verification notes for related context.",
...
def _append_retrieval_guide(
    ...
    lines.append(
        "Use artifact search for durable Pactile specs, tasks, research, "
