...
class KernelCliNotFound(RuntimeError):
    """`pactile kernel --json` (or PACTILE_KERNEL_CLI) is not available."""
...
def kernel_cli_argv() -> list[str]:
    override = os.environ.get("PACTILE_KERNEL_CLI", "").strip()
    if override:
    ...
    pactile = shutil.which("pactile")
    if pactile:
        return [pactile, "kernel", "--json"]
    raise KernelCliNotFound(
        "Kernel CLI not found. Set PACTILE_KERNEL_CLI or put `pactile` on PATH. "
        "Stage 2 create/start-execution/record-gate/archive/patch require `pactile kernel --json`."
    )
