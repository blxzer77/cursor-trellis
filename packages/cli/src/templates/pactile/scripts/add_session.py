...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage Pactile-owned workspace + task paths and commit.
    ...
    `.pactile/` tree, and if `.gitignore` blocks the specific paths we
    warn + skip — never retry with ``-f``.

    Honors ``session_auto_commit`` in ``.pactile/config.yaml``: when set to
    ...
    paths = safe_pactile_paths_to_add(repo_root)
    if not paths:
