...
// Write recording
//
// `pactile init` uses recording to capture exactly which files were actually
...
export async function writeFile(
  ...
  if (existingContent === content) {
    ...
    // byte-identical to a Pactile template and still not be Pactile-owned.
    return false;
  }
  ...
  if (mode === "skip") {
    ...
    // Skipped: Pactile did not write this file — caller should not track it
  ...
  if (mode === "append") {
    ...
    // Append: Pactile added managed content to a user-owned file. Tracking
