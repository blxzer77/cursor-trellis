...
| [Projection](projection-and-ownership.md#projection) | A rebuildable host-specific view of one canonical generation.               | Express Cursor/Codex files, managed blocks, and external bindings.                           | Cannot become canonical state or write inside `.pactile/`.                                   | Attach Cursor after a project was initialized for Codex without reauthoring the project truth. |
| [Ownership](projection-and-ownership.md#ownership)   | The ledger of resource owner, control, snapshots, claimants, and conflicts. | Decide whether reconcile, detach, restore, or remove is safe.                                | A claimant never gains deletion rights over borrowed, foreign, unknown, or modified content. | Detach Cursor while keeping a shared Skill still claimed by Codex.                             |
...
| `.cursor/`, optional `.codex/`, managed `AGENTS.md`, Pactile-owned shared Skills | Adapter projections | Yes, subject to ownership checks; Codex leaves are conditional on native support. |
