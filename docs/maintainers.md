...
1. Sync dogfood copies: `Trellis/.trellis/scripts/common/codebase_retrieval_router.py`, `common/project_file_stats.py`, `common/retrieval_tool_classification.py`, `common/git_context.py`, and `route_codebase_retrieval.py` from the CLI template (or run `trellis update` in `Trellis/`).
2. Harness MyHarness: keep the same paths under `.trellis/scripts/` aligned with the template (`project_file_stats.py` for `--project-file-count auto`).
