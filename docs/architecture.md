...
2. **Thin always-on pointer** — `.cursor/rules/cstl-bootstrap.mdc` (`alwaysApply: true`) names the four retrieval intents. Full how-to is `.cstl/framework/retrieval-daily-guide.md`. `retrieval-routing.mdc` is retired.
