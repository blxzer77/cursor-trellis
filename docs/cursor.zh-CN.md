...
Trellis 内置两个硬门禁,确保 dogfood 文件(`./cursor/` 与 `./.trellis/scripts/`)与生成模板一致。它们作为 `trellis init` / `trellis update` 流程的一部分,也可独立运行:

- **`trellis validate-rules`** —— 将 `.cursor/rules/` 下每个规则文件与 `packages/cli/src/templates/cursor/fixtures/expected-rules.ts` 中的清单比对。规则缺失、标题不符或不同步时,命令失败。
- **`pnpm mirror-check`**(贡献者侧)—— 比对 agent 与 rule 模板文件与仓库内 dogfood 实例,确保源模板与线上 `.cursor/` 文件不漂移。

`trellis init` 与 `trellis update` 在写入前调用 `assertCursorRulesValid()`,清单回归会中止操作,而非让项目处于半初始化状态。手动编辑 `.cursor/rules/` 后可运行 `trellis validate-rules` 复查。
...
- **`--check-compat`** —— 预检,确认已装 Cursor++ 构建中 `WPeLc8` resolver 符号仍在,再尝试任何 patch。
- **`smoke.py`** —— 健康检查,确认 patched 的 resolver 把 `subagentType` → slug,不读取任何含 secret 的文件(不查 provider key、不查 token)。
- **Native 安全忽略** —— `trellis init --cursor`(不带 `--cursor2plus`)对 Native API 用户打印一行提示:Cursor++ 附录可安全忽略。
