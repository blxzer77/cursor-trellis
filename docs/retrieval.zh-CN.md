...
| `unknown` | 保守 fallback | 视为 **BYOK**(若 fast-context MCP 可用则要求用它)**并**告警 `cursorEnv` 未解析;不要静默选 native |

路由信封始终包含 `cursorEnv`,Agent 据此知道调用哪个语义后端。这也是驱动双环境子 Agent 派发的同一信号——见 [Cursor 集成](cursor.zh-CN.md)。

### 检索合规(0.2.8 起)

检索路由器与 Agent 指令构建器强制环境合规:

- **`unknown` 保守** —— `cursorEnv` 无法解析时,路由器**不**默认 native `@codebase`;它走 BYOK 行为(fast-context MCP)并发出 `cursorEnv: unknown` 告警,让 Agent 询问用户,而非静默用一个可能不可用的后端。
- **LSP 过度承诺软化** —— Agent 指令不再声称 LSP / `GO_TO_DEFINITION` 可用。Cursor Agent 不暴露 LSP。定义跳转路由到 **codegraph**(`codegraph_node` / `codegraph_search`)再用 **Read** 取原文。指令含明确 caveat:LSP 不可用,codegraph 是替代。
- **Telemetry 拆分** —— 执行 telemetry 信封区分**planned**语义后端(路由器让 Agent 调什么)与**executed**语义后端(Agent 实际调了什么)。这能暴露 Agent 忽略路由决策的情况(如 BYOK 下调 `@codebase`,而它不存在)。
...
**LSP caveat(0.2.8 起):** Cursor Agent 不暴露 LSP / `GO_TO_DEFINITION`。Agent 指令构建器现明确声明这点,并把定义跳转路由到 `codegraph_node` / `codegraph_search` 再 `Read` 取原文。旧文档中"用 LSP"的指引视为已被取代。
