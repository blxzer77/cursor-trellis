...
| --- | --- | --- | --- | --- |
| **Internal Skill** | `templates/common/skills/*.md` (single-file) or `templates/common/bundled-skills/*/SKILL.md` (bundled) | Auto-triggered by workflow matcher | No | `cstl-brainstorm`, `cstl-micro-grill` |
| **Subagent** | `templates/cursor/agents/*.md` (frontmatter + system prompt) | Spawned by main session via `Task` tool | No | `cstl-research`, `cstl-implement`, `cstl-check` |
| **User Command** | `templates/common/commands/*.md` or `templates/cursor/commands/*.md` | Manually invoked by user via `/` | Yes | `/cstl-continue`, `/cstl-finish-work` |
...
| **Triggers** | Phase 1 planning; `cstl-start` routes "new feature / unclear requirement"; no selected task yet |
...
| **Boundaries** | Does NOT spawn legacy grill subagents; PRD Grill is in-session. Dispatches `cstl-research` Agent only for dedicated `research/<topic>.md` files |
...
| **Triggers** | Phase 2.1 before writing code; `cstl-start` routes "about to write code"; selected task `status=in_progress` |
...
| | |
| --- | --- |
| **Definition** | `templates/common/skills/check.md` (skill form) + `templates/cursor/agents/cstl-check.md` (agent form) — **dual form** |
| **Triggers** | Phase 2.2 / 3.1 quality check; `cstl-start` routes "finished coding / quality check" |
...
| | |
| --- | --- |
| **Definition** | `bundled-skills/cstl-micro-grill/SKILL.md` |
...
| **Triggers** | Any external/web/current-fact retrieval; `cstl-research` agent external search; `workflow.md` Discovery/Research stage; `retrieval-daily-guide` routing |
...
| --- | --- | --- |
| 1 | `cstl-brainstorm` | Phase 1 planning / `cstl-start` |
| 2 | `cstl-before-dev` | Phase 2.1 / `cstl-start` |
| 3 | `cstl-check` | Phase 2.2/3.1 / `cstl-start` |
| 4 | `cstl-break-loop` | `cstl-start` (stuck) |
| 5 | `cstl-update-spec` | Phase 3.3 learning decision |
| 6 | `cstl-micro-grill` | Triage Micro-Grill mode |
| 7 | `cstl-meta` | Local architecture modification |
...
| --- | --- | --- | --- |
| `cstl-research` | No | Yes (`cursor/agents/cstl-research.md`) | Always the Agent form — there is no skill by this name. Spawned when a topic needs a dedicated `research/<topic>.md` file |
| `cstl-implement` | No | Yes (`cursor/agents/cstl-implement.md`) | Always the Agent form. Spawned in Phase 2.1 to do the implementation work in its own context |
| `cstl-check` | Yes (`common/skills/check.md`) | Yes (`cursor/agents/cstl-check.md`) | **Dual form.** Skill form = main-session inline check (default, lighter). Agent form = independent review pass with self-fix capability (when a dedicated pass is needed). `workflow.md` recommends the Agent form when verifying after code changes |
...
- [Spec system design](spec-system.md) — what `cstl-spec-bootstrap` creates and `cstl-update-spec` maintains
