...
`blxzer77` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/blxzer77/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/blxzer77/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee blxzer77` to
...
```bash
python ./.trellis/scripts/task.py archive 00-join-blxzer77
```
